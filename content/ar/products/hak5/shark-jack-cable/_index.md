---
title: "HAK5 Shark Jack Cable — أداة تدقيق شبكي محمولة بشكل كابل"
description: "HAK5 Shark Jack Cable، أداة تدقيق شبكي محمولة مدمجة في كابل USB-C، بنفس وظائف Shark Jack مع سهولة حمل أكبر."
date: 2026-03-12
draft: false
showBreadcrumbs: true
brands: ["hak5"]
tags: ["تدقيق الشبكة", "إيثرنت", "USB-C", "اختبار الاختراق"]
dir: rtl
languageDirection: rtl
---

{{< alert "warning" >}}
**إشعار الاستخدام القانوني**: Shark Jack Cable أداة بحث أمني متخصصة، مخصصة حصراً للمتخصصين الأمنيين الحاصلين على تفويض كتابي قانوني.
{{< /alert >}}

## نظرة عامة على المنتج
Shark Jack Cable نسخة كابل من Shark Jack، تدمج وظائف التدقيق الشبكي في هيكل كابل USB-C، ويمكن توصيله مباشرة بمنفذ USB-C في الحاسوب المحمول، مما يجعله أسهل حملاً وأكثر إخفاءً من الإصدار الأصلي. يشارك نفس الوظائف مع Shark Jack ويدعم سكريبتات BashBunny.

{{< gallery >}}
  <img src="/images/products/hak5/shark-jack-cable.png" alt="HAK5 Shark Jack Cable" />
{{< /gallery >}}

---
{{< alert >}}
هل أنت مهتم بهذا المنتج؟ [تواصل معنا](/ar/contact/) للحصول على عروض الأسعار.
{{< /alert >}}
