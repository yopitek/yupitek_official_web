---
title: "Contacto | Yopitek Ltd"
description: "Póngase en contacto con el equipo de Yopitek Ltd. Respondemos en 1-2 días hábiles. Precios de productos, soluciones de proyección, consultoría técnica y servicios de mantenimiento."
date: 2026-03-12
draft: false
showBreadcrumbs: true
---

## Contáctenos

Bienvenido a Yopitek Ltd (榆閤科技有限公司). Nuestro equipo responde habitualmente en **1-2 días hábiles**.

---

## Información de Contacto

- **Persona de contacto:** Eason Lai
- **Correo electrónico:** [eason@yupitek.com](mailto:eason@yupitek.com)
- **Teléfono de oficina:** +886-2-87325338
- **Teléfono móvil:** 0926-881156
- **Dirección:** No.34, Lane 72, Fuyang St., Taipéi, Taiwán
- **Horario de atención:** Lunes – Viernes, 09:00 – 18:00 (GMT+8)

---

## Temas de Consulta

Estamos disponibles para responder sus consultas sobre los siguientes temas:

- **Precios de productos** — Solicitud de precios y cotizaciones de nuestro catálogo de productos
- **Soluciones de sistemas de proyección** — Planificación y diseño de proyectos de mapping arquitectónico y espacios inmersivos
- **Consultoría técnica** — Asesoría sobre equipos de TI y dispositivos de seguridad informática
- **Servicios de mantenimiento** — Mantenimiento, inspección y reparación de sistemas instalados

---

## Nota sobre Visitas

> **Aviso importante:** Yopitek no cuenta con una tienda física abierta al público. Si desea reunirse con nosotros en persona, le pedimos contactarnos previamente por teléfono o correo electrónico para coordinar una cita.
