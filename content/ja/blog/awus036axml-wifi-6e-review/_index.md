...
tags: ["AWUS036AXML", "wifi-6e", "レビュー", "Kali-Linux", "MT7921AU", "6GHz"]
---

## 製品概要

**ALFA AWUS036AXML** は、ALFA Network の Wi-Fi 6E 時代への参入を示すアダプターです。**MediaTek MT7921AU** チップセットを採用しており、2026 年時点で、セキュリティ研究者が Wi-Fi 6E ネットワークで使用する最新の無許可スペクトラム割り当てである **6 GHz 帯**での動作を可能にする、数少ない USB 無線アダプターの一つです。
...
セキュリティ研究者にとって最も重要なセクションです。MT7921AU のドライバー状況はチップセット登場以来大幅に改善されていますが、依然として注意が必要です。
...
MT7921AU にはバイナリファームウェアファイルが必要です。Kali Linux では通常 `firmware-misc-nonfree` パッケージでインストールできます：
...
レガシーバンドでの性能は AWUS036ACM（MT7612U）とほぼ同等か若干上回ります。MT7921AU の AX 機能は AC 世代のアダプターと比べてペネトレーションテストで直接的な優位性をもたらすわけではありませんが、最新カーネルでのよりクリーンなドライバー実装により、長時間の airodump-ng セッション中のキャプチャドロップが少なくなります。
