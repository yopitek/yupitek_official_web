---
title: "ALFA AWUS036AXER — Adaptador USB Wi-Fi 6 Nano Ultra-Slim"
description: "ALFA AWUS036AXER com chipset Realtek RTL8832BU. Wi-Fi 6 de banda dupla, fator de forma nano (~65×24×10mm). Para conectividade diária — não recomendado para Kali Linux ou pesquisa de segurança."
date: 2026-03-12
draft: false
showBreadcrumbs: true
showTableOfContents: true
brands: ["alfa"]
tags: ["Wi-Fi 6", "802.11ax", "Ultra-Slim", "USB 3.2", "Portátil", "Nano"]
---

## Visão Geral do Produto

O AWUS036AXER apresenta o chipset Realtek RTL8832BU com Wi-Fi 6 (802.11ax) de banda dupla (2.4 GHz + 5 GHz), até 1800 Mbps (2.4 GHz: 573 Mbps + 5 GHz: 1200 Mbps). Design nano ultra-slim (~65 × 24 × 10 mm, ~10g) para uso portátil cotidiano.

> ⚠️ **Nota:** Fator de forma nano — **sem conector RP-SMA**, antena não pode ser atualizada. **NÃO recomendado para Kali Linux ou pesquisa de segurança**.

> **Aviso macOS:** Todos os adaptadores ALFA têm suporte limitado ou nenhum para macOS. macOS 11 Big Sur e posteriores, e Apple Silicon (M1/M2/M3) **NÃO** são suportados. O suporte máximo é macOS 10.15 Catalina em Macs Intel.

## Características Principais

- Wi-Fi 6 (802.11ax) Banda Dupla: 2.4 GHz + 5 GHz
- Chipset Realtek RTL8832BU
- Até 1800 Mbps
- Design nano ultra-slim (~65×24×10mm, ~10g)
- USB 3.2 Gen 1 Type-A
- WPA3/WPA2/WPA/WEP
- ⚠️ Sem conector RP-SMA, antena integrada

## Especificações Técnicas

| Item | Especificação |
|------|---------------|
| Chipset | Realtek RTL8832BU |
| Padrões WiFi | IEEE 802.11 a/b/g/n/ac/ax (WiFi 6) |
| Bandas de frequência | 2.4 GHz + 5 GHz (sem 6 GHz) |
| Velocidade máxima | 1800 Mbps (2.4G: 573 Mbps + 5G: 1200 Mbps) |
| Antena | Nano integrada (sem RP-SMA) |
| Interface USB | USB 3.2 Gen 1 Type-A |
| Dimensões | ~65 × 24 × 10 mm, ~10g |
| Segurança sem fio | WPA3 / WPA2 / WPA / WEP |

## Suporte de SO

| SO | Status | Notas |
|----|--------|-------|
| Windows 10/11 | ✅ Suportado | Driver do site da Alfa |
| macOS | ❌ Não suportado | Sem suporte para macOS 11+ ou Apple Silicon |
| Ubuntu | ⚠️ Requer driver | Integrado no kernel ≥ 6.14 (Ubuntu 24.10+); versões anteriores precisam de DKMS manual |
| Kali Linux | ⚠️ Limitado | Monitor mode limitado em kernels < 6.12; não recomendado para pen testing |
| NetHunter | ⚠️ Limitado | Dependente do kernel |

## Hardware Compatível

| Hardware | Status | Notas |
|----------|--------|-------|
| Raspberry Pi 4/5 | ⚠️ Requer driver | Instalação manual no Pi OS com kernel < 6.14 |
| PC Desktop/Laptop | ✅ Suportado | USB-A padrão |

## Capacidades Avançadas

| Função | Status |
|--------|--------|
| Monitor Mode | ⚠️ Limitado |
| Packet Injection | ⚠️ Limitado |
| Soft AP Mode | ✅ Sim |
| Bluetooth | ❌ Não |

## Conteúdo da Caixa

- 1× Adaptador nano AWUS036AXER

## Recursos e Links

| Recurso | Link |
|---------|------|
| Documentação oficial | https://docs.alfa.com.tw/ |
| Driver Linux (RTL8832BU) | https://github.com/morrownr/rtl8852bu-20240418 |

## Download da Ficha Técnica

| Documento | Download |
|------|------|
| Ficha técnica oficial (PDF) | [📄 Baixar ficha técnica do AWUS036AXER](/docs/alfa/AWUS036AXER_spec.pdf) |

{{< gallery >}}
  <img src="/images/products/alfa/awus036axer_image_1.png" alt="ALFA AWUS036AXER" />
{{< /gallery >}}

---

{{< alert >}}
Precisa de um orçamento? [Fale conosco](/pt/contact/)
{{< /alert >}}
