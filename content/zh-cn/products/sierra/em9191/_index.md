---
title: "EM9191 5G NR Sub-6 + mmWave 蜂窝模块"
description: "EM9191 采用 Qualcomm SDX55 芯片组，支持 5G NR Sub-6 与 mmWave (n260/n261)，M.2 封装，适用于最高階 5G 应用。"
date: 2026-07-29
draft: false
showBreadcrumbs: true
showTableOfContents: true
brands: ["sierra"]
tags: ["5G", "mmWave", "Sub-6", "Cellular", "Module", "EM9191", "SDX55", "M.2"]
---

{{< alert "warning" >}}
**法律免责声明**：本产品为蜂窝通讯模块，使用前请确保符合当地法规与电信监管要求。未经授权的修改或使用可能违反法律。
{{</alert >}}

## 产品概述

EM9191 是 Sierra Wireless 推出的旗舰级 5G NR 蜂窝模组，采用 Qualcomm SDX55 芯片组，同時支持 Sub-6 与 mmWave (n260 39GHz / n261 28GHz)，提供最全面的 5G 覆盖。

下载速度最高可达 2.5 Gbps (5G) / 2.0 Gbps (LTE Cat 22)，上传最高 900 Mbps (5G)。支持 6 组天线介面以滿足 mmWave 波束成型需求。

EM9191 支持所有 Sub-6 频段 (同 EM9190) 加上 mmWave 高频频段，搭配多频 GNSS 系统，适合需要最低延迟与最高频宽的工业 4.0、智慧医療与边緣運算应用。

## 主要特点

- 采用 Qualcomm SDX55 5G 数据机芯片
- 支持 5G NR Sub-6 + mmWave (n260/n261)
- 5G 下载高达 2.5 Gbps，上传 900 Mbps
- LTE Cat 22 下载高达 2.0 Gbps
- 支持 8×CA 与 mmWave 波束成型
- 6 组天线介面 (含 mmWave 天线)
- M.2 3042 Key B 工业级封装
- 多频 GNSS (GPS + GLONASS + BeiDou + Galileo + QZSS)

## 技术规格

| 参数 | 规格 |
|------|------|
| 芯片型号 | Qualcomm SDX55 |
| 蜂窝标准 | 5G NR Sub-6 + mmWave + LTE-A Pro Cat 22 |
| 下载速度 | 2.5 Gbps (5G) / 2.0 Gbps (LTE) |
| 上传速度 | 900 Mbps (5G) / 150 Mbps (LTE) |
| 载波聚合 | 8×CA (LTE) + NR CA |
| LTE 类别 | Cat 22 |
| 5G 支持 | ✅ 5G NR Sub-6 + mmWave (n260/n261) |
| mmWave 支持 | ✅ n260 (39 GHz) / n261 (28 GHz) |
| 接口 | USB 3.1 / PCIe Gen3 / I2S / UART / RGMII |
| 外形规格 | M.2 3042 Key B |
| 尺寸 | 42 × 30 × 2.3 mm |
| 重量 | 7.2 g |
| 工作温度 | -40°C ~ +85°C |
| GNSS | GPS / GLONASS / BeiDou / Galileo / QZSS |
| 地区 | 全球 (多频段) |
| 供电电压 | 3.135V ~ 4.4V |
| 天线接口 | 6 × IPEX MHF4 |
| 固件更新 | USB / FOTA |

## 操作系统支持

| 操作系统 | 状态 | 备注 |
|------|---------|------|
| Windows 10/11 | ✅ | 经 Sierra Wireless 驱动程序认证 |
| Linux (Kernel ≥ 5.10) | ✅ | 内核内置 MBIM / QMI_WWAN 驱动 |
| Android | ✅ | 可通过 USB OTG 连接 |
| OpenWrt | ✅ | 需安装 kmod-usb-net-qmi-wwan |
| Yocto / Buildroot | ✅ | 支持 SDK 集成 |

## 包装内容

- 1 × EM9191 蜂窝模块
- 1 × 文件包装

## 资源与链接

| 资源 | 链接 |
|------|------|
| 官方产品页面 | https://www.sierrawireless.com/products/em9191/ |

{{< gallery >}}
  <img src="/images/products/sierra/em9191.png" alt="Sierra Wireless EM9191" />
{{</gallery >}}

---

{{< alert >}}
需要询问产品报价？请来信[与我们联系](/zh-cn/contact/)
{{</alert >}}
