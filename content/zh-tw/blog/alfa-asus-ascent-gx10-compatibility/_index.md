---
title: "ALFA 無線網卡是否支援 ASUS Ascent GX10（GB10）"
date: 2026-09-03
draft: false
...
categories:
  - "邊緣 AI / GPU 伺服器"
description: "ASUS Ascent GX10 與 NVIDIA DGX Spark 共享相同的 GB10 硬體平台與 DGX OS 軟體環境，對 ALFA 網卡的相容性完全一致（判定母體：ALFA 現役 9 款 USB 網卡）。MediaTek 晶片機型（AWUS036ACM / ACHM / AXML / AXM，4 款）使用 in-kernel 驅動，開箱即用；Realtek 晶片機型（AWUS03..."
featureimage: ""
author: "benny-lai"
lastmod: 2026-09-03
---
...
客戶詢問：「ALFA 系列 USB 無線網卡能否在 ASUS Ascent GX10（NVIDIA GB10 Grace Blackwell）AI 超級電腦上使用？」

簡短結論：ASUS Ascent GX10 與 NVIDIA DGX Spark 共享相同的 GB10 硬體平台與 DGX OS 軟體環境，對 ALFA 網卡的相容性完全一致（判定母體：ALFA 現役 9 款 USB 網卡）。MediaTek 晶片機型（AWUS036ACM / ACHM / AXML / AXM，4 款）使用 in-kernel 驅動，開箱即用；Realtek 晶片機型（AWUS036ACH / ACS / EACS / AX / AXER，5 款）需在 ARM64 上編譯 out-of-tree 驅動。注意：GX10 的 USB 埠均為 USB Type-C（3 個資料埠 + 1 個 PD 輸入埠），ALFA 網卡（AXML 除外）需使用 USB-C to USB-A 轉接器。

## 2. 分析目標硬體規格架構

### 2.1 ASUS Ascent GX10 硬體規格

| 項目 | 規格 |
|---|---|
| 產品名稱 | ASUS Ascent GX10 |
...
| USB | 3× USB 3.2 Gen 2×2 Type-C（20Gbps，DP Alt Mode / DisplayPort 2.1）+ 1× USB 3.2 Gen 2×2 Type-C（PD 輸入，180W EPR PD3.1） |
| 顯示輸出 | 1× HDMI 2.1（可搭配 USB-C DP Alt Mode 做多螢幕輸出） |
| 有線網路 | 1× 10GbE RJ45 + NVIDIA ConnectX-7 SmartNIC（2× 200G QSFP112） |
| 無線網路 | Wi-Fi 7（MediaTek AW-EM637，2×2 MIMO）+ Bluetooth 5.4 |
| 作業系統 | NVIDIA DGX OS（基於 Ubuntu Linux，kernel 6.x） |
| 架構 | aarch64（ARM64） |
| 尺寸 | 150 × 150 × 51 mm（5.91 × 5.91 × 2.01 inch） |
| 重量 | 1.48 kg |
| 散熱 | ASUS 專利散熱系統（靜音風扇 + 熱導管） |
| 其他 | Kensington 防盜鎖孔 |

> ⚠️ 規格修正註記：原稿尺寸寫「150 × 150 × 50 mm」且無重量，經查核 ASUS 官方 techspec 為 **150 × 150 × 51 mm / 1.48 kg**，已修正。HDMI 版本依官方為 2.1（原稿寫 2.1b 已修正）。見第 10 節參考來源。
...
| 差異項 | ASUS GX10 | NVIDIA DGX Spark |
|---|---|---|
| 散熱設計 | ASUS 專利散熱系統 | NVIDIA 參考散熱 |
| 機構設計 | ASUS 客製化機殼 | NVIDIA 參考機殼 |
| 無線模組 | MediaTek AW-EM637（Wi-Fi 7） | 同級 Wi-Fi 7 模組 |
| 配件 | ASUS 原廠配件 | NVIDIA 原廠配件 |
...
GX10 的 4 個 USB 埠均為 Type-C：

- 3 個資料埠（支援 DP Alt Mode，可接螢幕）
- 1 個 PD 輸入埠（用於供電）

ALFA 全系列網卡（除 AXML 為 USB-C 外）均為 USB Type-A，需使用轉接器。
...
| 使用場景 | 建議機型 | 說明 |
|---|---|---|
| 一般無線上網（最簡單） | AWUS036ACM / ACHM | in-kernel 驅動，免編譯 |
| 無線滲透測試 / 監聽 / 注入 | AWUS036ACH 或 AWUS036ACM | 兩者均支援 Monitor + Injection |
| Wi-Fi 6E / 6GHz | AWUS036AXML / AXM | MT7921AUN in-kernel 驅動 |
| 不需要外接 WiFi | — | GX10 已內建 Wi-Fi 7，一般上網不需外接 |
...
| 供電 | ASUS GX10 原廠 USB-C 電源供應器（180W EPR PD3.1） |
...
### ALFA 現役機型 × ASUS Ascent GX10（GB10）相容性矩陣
...
判定依據：ASUS GX10 與 DGX Spark 共享相同的 GB10 硬體平台與 DGX OS（kernel 6.x, aarch64），相容性判定與 DGX Spark 完全一致。

## 7. 超詳細 Step by Step 設定步驟

ASUS GX10 的安裝步驟與 NVIDIA DGX Spark 完全相同。以下為精簡版，完整步驟請參考 [ALFA 無線網卡是否支援 NVIDIA DGX Spark](https://yupitek.com/zh-tw/blog/alfa-nvidia-dgx-spark-compatibility/) 第 7 節。

### 7.1 MediaTek 晶片機型（開箱即用）

- 使用 USB-C to USB-A 轉接器（AXML 可直插），將 ALFA 網卡插入 GX10 的 USB-C 資料埠
- 確認偵測：`lsusb`
...
| Realtek 驅動編譯失敗 | 交叉編譯設定錯誤 | 確認在 GX10 上原生編譯；Makefile 不應設定 CROSS_COMPILE |
...
- 內建 Wi-Fi 7 可能與外接衝突：GX10 已內建 Wi-Fi 7（MediaTek AW-EM637）
- AP 模式需手動設定：DGX OS 預設為開發環境
- 6GHz 法規限制：Wi-Fi 6E 可用性取決於法規區域
- 驅動更新依賴上游：Realtek out-of-tree 驅動由社群維護，kernel 更新後需重新編譯
- ASUS 硬體差異不影響相容性：散熱與機構設計差異不影響 USB WiFi 驅動相容性

反駁條件：以上判定以 DGX OS（Ubuntu 基底、kernel 6.x）為前提。若 ASUS 未來推出非 DGX OS 韌體（如自家 Android / 客製化系統版本），判定需重新驗證。

## 10. 參考來源 URL

| 來源 | 說明 | URL | 查核狀態 | 查核日期 |
|---|---|---|---|---|
| ASUS Ascent GX10 官方 Techspec | GX10 硬體規格（**150×150×51mm / 1.48kg** / USB 配置 / HDMI 2.1） | https://www.asus.com/ph/networking-iot-servers/desktop-ai-supercomputer/ultra-small-ai-supercomputers/asus-ascent-gx10/techspec/ | ✅ 已查核 | 2026-09-03 |
| ASUS Ascent GX10 官方商城（UK） | GX10 產品頁（150 × 150 × 51mm） | https://uk.store.asus.com/asus-ascent-gx105004-33389.html | ✅ 已查核 | 2026-09-03 |
...
相關文章：[ALFA 無線網卡是否支援 NVIDIA DGX Spark](https://yupitek.com/zh-tw/blog/alfa-nvidia-dgx-spark-compatibility/)｜[ALFA 無線網卡是否支援 ALTOS BrainSphere GB10 F1](https://yupitek.com/zh-tw/blog/alfa-altos-brainsphere-gb10-compatibility/)｜[ALFA 無線網卡是否支援 GIGABYTE AI TOP ATOM](https://yupitek.com/zh-tw/blog/alfa-gigabyte-ai-top-atom-compatibility/)｜[ALFA 無線網卡是否支援 MSI EdgeXpert](https://yupitek.com/zh-tw/blog/alfa-msi-edgexpert-compatibility/)

免責聲明：本文相容性判定以 ASUS Ascent GX10 預裝的 NVIDIA DGX OS（kernel 6.x, aarch64）為基準。GX10 與 DGX Spark 共享相同硬體平台，相容性完全一致。MediaTek 晶片驅動為 Linux mainline，穩定性高；Realtek 晶片驅動為社群維護。GX10 已內建 Wi-Fi 7，外接 ALFA 主要用於滲透測試或特殊晶片組需求。
