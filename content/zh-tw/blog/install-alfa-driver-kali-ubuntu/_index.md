...
Kali Linux 和 Ubuntu 都會定期更新核心。若未使用 DKMS，每次核心更新後非主線驅動程式（RTL8812AU）就會失效，直到你手動重新編譯為止。
...
3. **RTL8812AU?？** → Clone 對應儲存庫，執行 `make && sudo make install`，啟用 DKMS 確保持久運作
