...
categories:
  - "Wireless Hardware"
  - "Technical Guide"
description: "探討在 JetPack 6 (Ubuntu 22.04 LTS) 下以 AWUS036AXML 啟用 6GHz 頻段，為多路 4K RTSP 串流提供超低延遲且免受干擾的無線通道。"
featureimage: "/images/blog/07_jetson_6ghz_streaming.jpg"
author: "benny-lai"
lastmod: 2026-08-18
faq:
  - question: "為什麼 6GHz 頻段對多路 4K 串流比 5GHz 更具優勢？"
    answer: "6GHz 頻段擁有更寬廣且無舊式 Wi-Fi 設備競爭的乾淨頻寬，並具備 160MHz 大頻寬通道，顯著降低傳輸延遲與抖動。"
---
...
把 Jetson Orin Nano 接到基地台（AP）跑兩三路 IP 鏡頭，看起來很平常——但實際把多路 **4K 即時影像**送進 GPU 做推論時，很多人才第一次感受到無線網路真正的極限：

- 畫質一直掉碼（bitrate 上不去，畫面起霧、馬賽克）。
- 延遲忽高忽低，影像 AI 模型推論的「時間錯位」愈來愈明顯。
- 排程卡住，集控端畫面黑屏，一查是「無線掉包」。

這篇文章要用**物理層 → 設定層 → 量測層**三個角度，把「邊緣端多路 RTSP 4K 串流」的頻寬挑戰拆乾淨，並示範如何在 **Jetson Orin Nano（JetPack / Ubuntu 22.04 LTS）** 上外接 **AWUS036AXML Wi-Fi 6E 網卡**、切換到乾淨的 **6GHz 頻段**，再用數據證明「為什麼 6GHz 是這類工作量的首選」。

如果你還沒決定要不要買這張卡，建議先跳去最後一章的「採購前相容性確認工作表」逐項打勾。
...
> **重點**：4K 是「每路就吃掉 ~2–4 成 HD 流量」的怪物。四路 4K/H.264 同時進板，等於 80–140 Mbps 的「有效承載量」。注意是**有效承載**，不是無線 PHY 速率——兩者差了快一倍的距離（見 1.3）。

### 1.2 掉包 ≠ 訊號問題：無線媒介是半雙工且共享的

很多人以為「訊號看起來滿格就沒問題」，但在邊緣場域，真正的殺手是**擁堵**：

- **2.4GHz 只剩 3 個不重疊通道**：藍牙、微波爐、隔壁廠房的 AP 全擠在這裡，CSMA/CA 的退避（backoff）機制讓「同時有幾台裝置」時吞吐直接腰斬再腰斬。
...
所以一張宣稱 3000 Mbps class 的網卡，**在乾淨環境下的真實承載量通常落在 600–900 Mbps**，這對多路 4K（80–140 Mbps）綽綽有餘；但**一旦塞進擁堵的 2.4G/5G 通道，實測往往只剩 100–300 Mbps**——直接卡爆。
...
AWUS036AXML 的核心優勢是 **MediaTek MT7921AU 的 `mt7921u` 驅動已原生整合進 Linux 主線核心**（自 Kernel 5.18 起收錄），**不需要去 GitHub 編譯驅動**。但「原生支援」有門檻，先確認你的核心版本：
...
|---|---|---|---|
| JetPack 5.1.x | Ubuntu 20.04/22.04 | 5.10 | 需自行確認驅動；建議直接升上 JetPack 6.x |
...
> **監理法規提醒**：6GHz 頻段是否可用取決於你所在國家/地區的監理法規與**基地台韌體**（例如台灣已依 NCC 開放 Wi-Fi 6E 使用 6GHz）。若 `iw reg get` 顯示的 regulatory domain 未開放 6GHz，或 AP 未啟用 6GHz，網卡是連不上的——這不是硬體故障，是法規/設定問題。
...
| 頻段 | PHY bitrate | iperf3 實測上下行 | ping 平均/抖動 | 2×4K 串流結果 |
|---|---|---|---|---|
...
> 這是「乾淨 vs 擁堵」的典型對比。**6GHz 的價值不在「比 5GHz 快」本身，而在於它是全新的、幾乎無人使用的頻段**——在鏡頭密集、Wi-Fi 裝置爆量的場域，這個優勢立刻變成穩定的多路 4K 承載力。
