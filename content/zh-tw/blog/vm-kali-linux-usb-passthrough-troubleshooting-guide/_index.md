...
description: "VirtualBox/VMware 虛擬機 USB 穿透排障手冊：Extension Pack、USB 3.0 (xHCI) 控制器、vboxusers 群組、USB 過濾器，以及 lsusb→iwconfig→dmesg 三層診斷流程。"
date: 2026-08-31
author: benny-lai
lastmod: 2026-08-31
draft: false
...
faq:
  - question: "我換了 USB 連接埠結果 `lsusb` 就消失了，是網卡壞了嗎？"
    answer: "不一定。先確認你插的是不是「僅充電」連接埠，或主機為了省電把裝置休眠了。換回主機板背板的一般 USB 連接埠，或重新插拔一次，多半就恢復。"
  - question: "VM 視窗右下角 USB 圖示是空的，怎麼辦？"
    answer: "依序檢查：① 擴充套件版本是否與 VirtualBox 完全相符；② Linux 主機是否在 `vboxusers` 群組（需重新登入）；③ 主機端 `lsusb` 還看不看得到網卡；④ 是否有其他軟體（例如主機端驅動工具）佔用了裝置。"
  - question: "設定 USB 過濾器後，主機自己反而不能用網卡了？"
    answer: "這是正常的。穿透給 Guest 後，裝置控制權在 Guest 身上，主機端無法同時使用。要用回主機做其他事情時，先在 VM 視窗的 USB 圖示把它「退還（release）」回主機。"
  - question: "Guest 內 `lsusb` 看得到，但沒 wlan 介面，該裝什麼驅動？"
    answer: "看晶片：AWUS036AXML（MediaTek MT7921AU）：核心內建 `mt7921u` 驅動，Kernel 5.18+ 隨插即用；先確認 `apt install linux-firmware` 已更新。AWUS036ACH（Realtek RTL8812AU）：屬於核心外掛（out-of-tree）驅動，需安裝社群維護的 `aircrack-ng/rtl8812au` 並用 DKMS 編譯（並留意 Secure Boot 的 MOK 簽署，請勿關閉 Secure Boot）。"
  - question: "為什麼選了 USB 3.0 控制器 Guest 反而進不了系統？"
    answer: "少數舊版 Guest 核心對 xHCI 的支援較差。若 Kali 是較舊版本，可先試「關機 → 改回 USB 2.0 (EHCI) Controller → 開機 → 升級核心 → 再改回 USB 3.0」。盡量讓 Kali 保持最新版本，xHCI 支援較完整。"
  - question: "網卡在真機上很快，進虛擬機就變慢，是正常的嗎？"
    answer: "是的，虛擬機內的網卡效能大致等同「透過 USB 模擬層的轉送」，會比真機直插多一些損耗（overhead）；正確的 USB 3.0 (xHCI) 控制器與更新版 Hypervisor 能把損耗壓到最低。若效能嚴重低落，優先確認控制器不是停在 USB 1.1。"
---
...
很多 Kali 使用者把網卡插在主機上，卻在虛擬機裡看不到無線介面。**這種情況多半出在三個非常常見的原因**，網卡本身壞掉的機率很低：
...
排障順序：先主機端硬體、再 Guest 端穿透、最後查驅動層——完整診斷口訣見 1.3。

{{< /tldr >}}

---

## 1. 為什麼虛擬機預設用不到主機的無線網卡？
...
穿透成功後，這個裝置在**主機端的控制權會轉給 Guest**，行為上像被「拔走」，主機上無法再使用它。轉而在 Guest 內變成一個全新的 USB 裝置。**這是正常現象，不是 bug。** 主機的一個 USB 裝置不能同時給兩邊用。
...
> 安裝時會顯示 Oracle 授權（Personal Use and Evaluation License）；個人使用免費，商用環境請依授權內容辦理。
...
如果清單是空的，代表穿透層出了問題——回去 2.2 / 2.3 / 2.4 檢查（含 USB 控制器未啟用），或直接跑第六章排障工作表。

**VMware 對照**：VMware Workstation / Fusion **不需要**額外擴充套件就有 USB 穿透功能，但有兩個常見檢查點：
...
### 4.2 頻繁斷線：多半是供電或控制器問題

高功率網卡（AWUS036ACH 監聽／注入時瞬時電流較高；AWUS036AXML 為 USB 3 規格）在虛擬機內偶發「用一用就掉卡／斷線」。以下是典型原因與對策：
...
是的，虛擬機內的網卡效能大致等同「透過 USB 模擬層的轉送」，會比真機直插多一些損耗（overhead）；正確的 USB 3.0 (xHCI) 控制器與更新版 Hypervisor 能把損耗壓到最低。若效能嚴重低落，優先確認控制器不是停在 USB 1.1。

---

## 8. 結論與硬體建議

「虛擬機抓不到外接網卡」九成以上是**穿透設定**或**Guest 驅動**沒做好，硬體故障反而少見。把本文的動作照順序跑完：
