---
title: "HAK5 Shark Jack Cable — Cable 版攜帶式網路稽核工具"
description: "HAK5 Shark Jack Cable，整合於 USB-C 線材的攜帶式網路稽核工具，功能同 Shark Jack，攜帶更方便。"
date: 2026-03-12
draft: false
showBreadcrumbs: true
brands: ["hak5"]
tags: ["網路稽核", "乙太網路", "USB-C", "滲透測試"]
---

{{< alert "warning" >}}
**合法使用聲明**：Shark Jack Cable 為專業資安研究工具，僅供具備合法書面授權的資安專業人員使用。
{{< /alert >}}

## 產品概述

Shark Jack Cable 是 Shark Jack 的線材整合版本，將網路稽核功能整合於 USB-C 線材形式中，可直接插入筆電 USB-C 埠使用，比原版 Shark Jack 更便於攜帶隱藏。功能與 Shark Jack 相同，支援 BashBunny 腳本。

{{< gallery >}}
  <img src="/images/products/hak5/shark-jack-cable.webp" alt="HAK5 Shark Jack Cable" />
{{< /gallery >}}

---

{{< alert >}}
需要詢問報價？[聯絡我們](/zh-tw/contact/)
{{< /alert >}}
